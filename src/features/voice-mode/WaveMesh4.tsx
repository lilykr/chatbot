...
interface WaveMesh4Props {
	radius?: number;
	pointCount?: number;
	color?: string;
	waveIntensity?: number;
	waveSpeed?: number;
	rotationSpeed?: number; // Duration in ms for one full rotation
	colorThreshold?: number; // Point at which color starts becoming white (0-1)
}

export function WaveMesh4({
	radius = 100,
	pointCount = 1000,
	color = "#FF00FF",
	waveIntensity = 15,
	waveSpeed = 2000,
	rotationSpeed = 20000,
	colorThreshold = 0.3, // Above this opacity, color will start becoming white
}: WaveMesh4Props) {
	...
	// Add rotation angles for sphere rotation
	...
	useEffect(() => {
		...
		// Create smooth, constant rotation animations
		const baseRotationDuration = rotationSpeed;

		// X rotation: base speed
		...
		// Z rotation: even slightly faster
	...
	const interpolateColor = useCallback(
		(baseColor: string, opacity: number) => {
			...
			if (opacity > colorThreshold) {
				// Calculate how far we are between threshold and 1
				const t = (opacity - colorThreshold) / (1 - colorThreshold);

				// Instead of going to pure white (255), we'll go 70% towards white
				const targetR = r + (255 - r) * 0.7;
				const targetG = g + (255 - g) * 0.7;
				const targetB = b + (255 - b) * 0.7;

				// Interpolate towards lighter color
	...
	// Memoize point rendering function
	const renderPoint = useCallback(
		(point: Point3D, index: number) => {
			const cx = useDerivedValue(() => {
				...
				// Calculate wave effect on original (unrotated) coordinates
				const calculateWave = (progress: number, angle: number) => {
				...
				const wave1 = calculateWave(progress1.value, 0);
				const wave2 = calculateWave(progress2.value, Math.PI / 3);
				const wave3 = calculateWave(progress3.value, -Math.PI / 4);
				...
				// Combine rotation with wave effect
				return rotated.x + centerX + totalOffset * (point.originalX / radius);
			}, []);
			...
			const cy = useDerivedValue(() => {
				...
				// Combine rotation with wave effect
				return rotated.y + centerY + totalOffset * (point.originalY / radius);
			}, []);
			...
			const opacity = useDerivedValue(() => {
				...
				// Use rotated Z for opacity calculation with more dramatic contrast
				const normalizedZ = (rotated.z + radius) / (2 * radius);
				const contrast = 2.5; // Increased contrast
				const minOpacity = 0.05;
				const maxOpacity = 1;
				return Math.max(minOpacity, normalizedZ ** contrast * maxOpacity);
			}, []);

			const pointColor = useDerivedValue(() => {
				"worklet";
				return interpolateColor(color, opacity.value);
			}, [color, opacity]);
	...
	return (
		<Canvas style={styles.canvas}>
			<Group>
				{spherePoints.map((point, index) => renderPoint(point, index))}
			</Group>
		</Canvas>
	);
}
